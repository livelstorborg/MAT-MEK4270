...
class Legendre(FunctionSpace):
    ...
    def L2_norm_sq(self, N):
        return 2.0 / (2.0 * np.arange(N) + 1.0)
...
class NeumannLegendre(Composite, Legendre):
    def __init__(self, N, domain=(-1, 1), bc=(0, 0), constraint=0):
        Legendre.__init__(self, N, domain=domain)
        self.B = Neumann(bc, domain, self.reference_domain)
        # Create stencil matrix with j(j+1)/((j+2)(j+3)) coefficients
        j = np.arange(N + 1)
        c = j * (j + 1) / ((j + 2) * (j + 3))
        self.S = sparse.diags((np.ones(N + 1), -c), (0, 2), shape=(N + 1, N + 3), format="csr")

    def basis_function(self, j, sympy=False):
        c = j * (j + 1) / ((j + 2) * (j + 3))
        if sympy:
            return sp.legendre(j, x) - c * sp.legendre(j + 2, x)
        return Leg.basis(j) - c * Leg.basis(j + 2)
...
class NeumannChebyshev(Composite, Chebyshev):
    def __init__(self, N, domain=(-1, 1), bc=(0, 0), constraint=0):
        Chebyshev.__init__(self, N, domain=domain)
        self.B = Neumann(bc, domain, self.reference_domain)
        # Create stencil matrix with j^2/(j+2)^2 coefficients
        j = np.arange(N + 1, dtype=float)
        c = np.where(j == 0, 0.0, j**2 / (j + 2)**2)
        self.S = sparse.diags((np.ones(N + 1), -c), (0, 2), shape=(N + 1, N + 3), format="csr")

    def basis_function(self, j, sympy=False):
        c = 0 if j == 0 else j**2 / (j + 2)**2
        if sympy:
            return sp.cos(j * sp.acos(x)) - c * sp.cos((j + 2) * sp.acos(x))
